...
public class OrganizationService extends GitHubService
{
	...
	public User editOrganization(User organization) throws IOException
	{
		...
		if(name == null)
			throw new IllegalArgumentException("Organization login cannot be null"); //$NON-NLS-1$
		if(name.length() == 0)
			throw new IllegalArgumentException("Organization login cannot be empty"); //$NON-NLS-1$
