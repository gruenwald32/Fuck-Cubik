...
public class OAuthService extends GitHubService
{
		...
		throws IOException
	{
		StringBuilder uri = new StringBuilder(SEGMENT_AUTHORIZATIONS);
		uri.append('/').append(id);
		Map<String, Collection<String>> params = Collections.singletonMap("add_scopes", scopes); //$NON-NLS-1$
		...
		throws IOException
	{
		StringBuilder uri = new StringBuilder(SEGMENT_AUTHORIZATIONS);
		uri.append('/').append(id);
		Map<String, Collection<String>> params = Collections.singletonMap("remove_scopes", scopes); //$NON-NLS-1$
		return client.post(uri.toString(), params, Authorization.class);
	}
		...
		throws IOException
	{
		StringBuilder uri = new StringBuilder(SEGMENT_AUTHORIZATIONS);
		uri.append('/').append(id);
		Map<String, Collection<String>> params = Collections.singletonMap("scopes", scopes); //$NON-NLS-1$
		return client.post(uri.toString(), params, Authorization.class);
	}
}
